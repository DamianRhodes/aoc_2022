...
bool stack_move2(stack_t *from, stack_t *to, size_t number) {
	bool fail = false;

	stack_t temp_stack = { .height = 0 };
	for (size_t i = 0; i < number; i++) {
		char moved_box;
		fail |= !stack_pop(from, &moved_box);
		fail |= !stack_push(&temp_stack, moved_box);
	}

	for (size_t i = 0; i < number; i++) {
		char moved_box;
		fail |= !stack_pop(&temp_stack, &moved_box);
		fail |= !stack_push(to, moved_box);
	}
	return fail;
}
		...
		if (creating_stacks) {
			...
			if (sscanf(line, "move %zu from %zu to %zu",
				...
				*/
				...
				//task2
				stack_move2(&stacks[instr.from], &stacks[instr.to], instr.count);
