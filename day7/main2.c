...
#define REQUIRED_DISK_SPACE 30000000
...
int main() {
	...
	unsigned root_size = get_dir_size(files, file_cnt, "/");
	printf("%u taken up in total\n", root_size);

	unsigned currently_free = TOTAL_DISK_SPACE - root_size;
	unsigned missing_free = REQUIRED_DISK_SPACE - currently_free;
	printf("%u needs to be freed up\n", missing_free);

	unsigned smallest_possible = root_size;

	unsigned folder_size_above_treshold_sum = 0;
	for (size_t i = 0; i < folder_cnt; i++) {
		unsigned folder_size = get_dir_size(files, file_cnt, folders[i].path);
		if (smallest_possible > folder_size && folder_size >= missing_free) {
			//printf("%s is below the treshold\n", folders[i].path);
			//folder_size_above_treshold_sum += folder_size;
			smallest_possible = folder_size;
		}
	}
	...
	printf("Smallest deletable folder is %u\n", smallest_possible);
